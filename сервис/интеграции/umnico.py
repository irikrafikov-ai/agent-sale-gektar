...
если диалога нет — «написать первым» в MAX по номеру.

ТОЛЬКО ТЕКСТ — и это решение, а не недоделка (25.09.2026). Проверено на
живом API: канал MAX (saId 119876) загрузку файла не принимает вовсе —
POST /v1.3/messaging/upload отвечает 500 на все попытки, включая кадр в
98 КБ. Телеграм (saId 122465) файл принимает (200, type=photo, fileId),
то есть вложения технически возможны там. Ирик решил: «пусть пока будет
так» — живые фото уходят в MAX ССЫЛКОЙ на папку Яндекс.Диска, поддержку
вложений в Телеграме не делаем. Возвращаться к теме без его слова не надо. Номер нормализуется
...
SA_TELEGRAM = int(os.environ.get("UMNICO_SA_TELEGRAM", "122465"))  # канал Telegram, тот же номер
КАНАЛЫ = {"max": SA_MAX, "telegram": SA_TELEGRAM}
...
class Umnico:
    ...
    def отправить(self, сырой_номер: str, текст: str, канал: str = "max") -> dict:
        """Одна точка входа: нормализовать номер → найти диалог → написать;
        диалога нет → написать первым в канал, который выбрал клиент.

        Канал спрашиваем у клиента (правило Ирика 25.09.2026): «вам удобнее
        в MAX или в Телеграм?». Существующий диалог сильнее выбора: если
        человек уже пишет нам в одном из каналов, отвечаем туда же, а не
        заводим вторую ветку разговора с тем же человеком.

        Телеграм по номеру может и не принять «первым» — тогда честная
        ошибка, а не молчаливый уход в MAX: агент должен знать, куда ушло
        сообщение, потому что клиенту он называет канал словами.
        """
        номер = нормализовать_номер(сырой_номер)
        if not номер:
            raise UmnicoError(f"не похоже на российский мобильный: {сырой_номер!r}")
        ключ = (канал or "max").strip().lower()
        if ключ not in КАНАЛЫ:
            raise UmnicoError(f"канал {канал!r} не знаю, есть: {', '.join(КАНАЛЫ)}")
        ...
        ответ = self.написать_первым(номер, текст, sa=КАНАЛЫ[ключ])
        назв = "MAX" if ключ == "max" else "Телеграм"
        return {"номер": номер, "как": f"первым в {назв}", "канал": ключ, "ответ": ответ}
