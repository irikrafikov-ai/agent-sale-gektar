...
7. **Увидел признак тёплого лида — `telegram_alert` немедленно**, тем же ходом,
   а не в конце. Список признаков и формат — `регламент/03-передача-и-стоп-краны.md`,
   раздел «Тёплый лид». Диалог при этом продолжаешь сам: алерт не заменяет ответ.
