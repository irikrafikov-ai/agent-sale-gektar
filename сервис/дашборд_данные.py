...
def расходы_авито(операции: list[dict]) -> dict[str, dict]:
    """Операции кошелька Авито → по дням: сколько потрачено (₽) и на что.

    Расход — всё, что не пополнение («аванс»); возврат зарезервированных
    средств вычитается в день возврата. Так «Рассылка предложений» 7 056 ₽
    с возвратом 2 226 ₽ через два дня даёт честные 4 830 ₽ по итогу.
    """
    итог: dict[str, dict] = defaultdict(lambda: {"руб": 0.0, "по_услугам": defaultdict(float)})
    for о in операции:
        тип = (о.get("operationType") or "").lower()
        if "аванс" in тип or "пополнен" in (о.get("operationName") or "").lower():
            continue
        сумма = float(о.get("amountTotal") or о.get("amountRub") or 0)
        if "возврат" in тип:
            сумма = -сумма
        д = _дата(о.get("updatedAt") or "")
        if not д:
            continue
        услуга = о.get("serviceName") or о.get("operationName") or "прочее"
        итог[д]["руб"] += сумма
        итог[д]["по_услугам"][услуга] += сумма
    return {д: {"руб": round(v["руб"], 2), "по_услугам": {k: round(x, 2) for k, x in v["по_услугам"].items()}}
            for д, v in итог.items()}
...
def динамика(
    ...
    for д in дни(конец, дней):
        ...
        ряд.append({
            "дата": д,
            "авито_руб": а.get("руб", 0.0),
            "авито_по_услугам": а.get("по_услугам", {}),
            "новых": новые.get(д, 0),
...
def итоги(ряд: list[dict]) -> dict:
    ...
    авито = round(s("авито_руб"), 2)
    по_услугам: dict[str, float] = defaultdict(float)
    for r in ряд:
        for k, v in (r.get("авито_по_услугам") or {}).items():
            по_услугам[k] += v
    return {
        "дней": len(ряд),
        "авито_руб": авито,
        "авито_по_услугам": dict(sorted(((k, round(v, 2)) for k, v in по_услугам.items()), key=lambda x: -x[1])),
        "авито_на_лид": round(авито / новых, 2) if новых else 0.0,
        "авито_на_тёплого": round(авито / тёплых, 2) if тёплых else 0.0,
        "новых": новых,
...
def _операции_авито(с_даты: str, сейчас: datetime) -> list[dict]:
    """Кошелёк Авито кабинета «Щёкинские берега». Нет ключей или сбой —
    дашборд живёт без этой колонки, а не падает."""
    import кабинеты as реестр
    from интеграции.avito import Avito

    каб = реестр.кабинет(реестр.ПО_УМОЛЧАНИЮ)
    if not реестр.настроен(каб):
        return []
    id_, secret = каб["переменные"]
    клиент = Avito(client_id=os.environ[id_], client_secret=os.environ[secret])
    try:
        return клиент.operations_history(f"{с_даты}T00:00:00", сейчас.strftime("%Y-%m-%dT%H:%M:%S"))
    except Exception as ошибка:  # noqa: BLE001
        print(f"[дашборд] операции Авито не получены: {type(ошибка).__name__}: {ошибка}", file=sys.stderr)
        return []
    finally:
        try:
            клиент.close()
...
def собрать(дней: int = 30, сейчас: datetime | None = None, свежие: bool = False) -> dict:
    ...
    try:
        ...
        b.close()
    операции = _операции_авито(с_даты, сейчас)

    ряд = динамика(сделки, история, журнал, сейчас, дней * 2, операции)
    ...
    данные = {
        ...
        # Отчёты ежедневные (Ирик, 21.09.2026) — история того, что уходило в Телеграм.
        "отчёты": [
            {"дата": о["дата"], "текст": о["текст"], "вывод": архив_отчётов.вывод_из_отчёта(о["текст"])}
            for о in архив_отчётов.прочитать(дней=90)
        ],
    }
