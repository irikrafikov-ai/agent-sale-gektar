...
ПЕРИОДЫ = (30, 7, 90)  # 30 — первым: его открывают чаще всего
...
def собрать(дней: int = 30, сейчас: datetime | None = None, свежие: bool = False) -> dict:
    ...
    # Битрикс по вечерам отвечает медленно (21.09: ReadTimeout на 30 с) —
    # дашборду спешить некуда, даём минуту на запрос.
    b = Bitrix(timeout=90.0)
    try:
...
def обновлять_в_фоне() -> None:
    ...
    Сбой сборки (таймаут Битрикса) — повтор через 20 с, а не через пять
    минут: пока кэш пуст, страница показывает «собираю данные».
    """
    while True:
        сбой = False
        for дней in ПЕРИОДЫ:
        ...
        time.sleep(20 if сбой else КЭШ_СЕКУНД)
