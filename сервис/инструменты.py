...
async def avito_send_message(args: dict) -> dict:
    ...
    if (просьба := просил_не_писать(история, cli.user_id)):
        ...
        return _err(
            "СТОП-КРАН: клиент просил больше не писать. Сообщение НЕ отправлено "
            "и отправлять его нельзя ни под каким поводом.\n\n"
            f"Его слова: «{цитата}»\n\n"
            "Правило Ирика 06.09.2026: попросил — молчим навсегда, даже если "
            "появился новый повод или пришёл свежий сигнал Авито. Закрой сделку "
            "в Битриксе (причина «Не интересно / передумал», в комментарий — "
            "цитату) и добавь в название пометку НЕ ПИСАТЬ."
        )
