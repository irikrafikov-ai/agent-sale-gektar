...
@tool(
    ...
    "Дубль по названию не создастся: если такая незакрытая задача уже есть, "
    "инструмент вернёт её ID и НЕ заведёт вторую — это не ошибка.",
...
async def b24_task_add(args: dict) -> dict:
    ...
    if not название:
        return _err("нужен title")

    # СТОП-КРАН ОТ ДУБЛЕЙ ЗАДАЧ (Ирик, 23.09.2026).
    #
    # 43 одинаковые задачи «Подхватить диалог: Тамара Ивановна — сделка 2415»
    # накопились к 23.09: у неё ТРИ чата, и каждый входящий в любой из них
    # поднимал разбор, а разбор честно заводил задачу — модель не обязана
    # помнить, что такая уже есть. Список задач у Ирика превратился в шум,
    # в котором тонут настоящие.
    #
    # Проверка здесь, в коде, а не в правилах брифа: правило можно забыть,
    # код — нет. Сравниваем по точному названию среди НЕЗАКРЫТЫХ задач.
    try:
        похожие = bitrix().task_list(filter={"TITLE": название, "!STATUS": "5"}, limit=5)
    except Exception:  # noqa: BLE001 — не смогли проверить, лучше завести
        похожие = []
    if похожие:
        есть = похожие[0]
        return _ok({
            "уже_есть": True,
            "id": есть.get("id") or есть.get("ID"),
            "пояснение": (
                f"Задача «{название}» уже стоит и не закрыта — вторую не завожу. "
                "Если нужно обновить срок или текст — правь существующую "
                "(b24_call tasks.task.update). Это не ошибка."
            ),
        })
...
@tool(
    "telegram_alert",
    "ТОЛЬКО тёплый/горящий лид. Больше ничего в Телеграм не уходит — "
    "ни находки, ни баги, ни сломанные инструменты, ни нарушения правил "
    "другими сервисами: всё это идёт в вечерний отчёт, раздел «Сбои» или "
    "«Требуют вас» (правило Ирика 09.09.2026). "
    "ОБЯЗАТЕЛЬНО передай chat_id и account: перед отправкой проверяется, "
    "что клиент вообще писал в этот чат.",
...
async def telegram_alert(args: dict) -> dict:
    ...
    # ТОЛЬКО ТЁПЛЫЙ ЛИД. Правило Ирика 09.09.2026: «технические отчёты в
    # телеграм слать не нужно, только уведомления, если сервер поломался».
    #
    # 09.09 туда ушли два подробных технических разбора: баг цен в шаблонах
    # follow-up-раннера и отказ схемы handoff_chat. Находки верные, но канал
    # не для них: Ирик читает его ради клиентов, и чем больше в нём техники,
    # тем вернее в ней потеряется живой человек.
    #
    # Про поломку сервера сообщает КОД, а не модель: падение прогона и
    # нехватка переменных окружения уже шлют алерт из main(). Модель об
    # аварии судить не должна — она сама может быть её частью.
    if not лид:
        return _err(
            "СТОП-КРАН: в Телеграм уходят ТОЛЬКО тёплые лиды. "
            "Сообщение НЕ отправлено — и это не ошибка.\n\n"
            "Находки, баги, сломанные инструменты, нарушения правил другими "
            "сервисами — всё это в ВЕЧЕРНИЙ ОТЧЁТ: раздел «Сбои», если что-то "
            "не работает, или «Требуют вас», если без Ирика не сдвинется. "
            "Там это увидят в собранном виде и в контексте дня.\n\n"
            "Про падение сервера Ирику сообщит код — сам, отдельным алертом. "
            "Тебе про аварию писать не нужно."
        )

    # ОДИН ЧЕЛОВЕК — ОДИН АЛЕРТ (Ирик, 22.09.2026: два алерта об Екатерине
    # из двух её чатов). Если это второй чат того же человека — смотрим и
    # память по основному чату.
    основной = (os.environ.get("ДУБЛЬ_ЧАТА") or "").split("|")[0]
    if лид and chat_id and (_уже_уведомляли(chat_id) or (основной and _уже_уведомляли(основной))):
        return _err(
...
@tool(
    "max_send",
    "Написать клиенту в MAX или Телеграм по номеру телефона (наш номер "
    f"{НОМЕР_КАНАЛА}). Номер в любом виде — «9207938710», «8 920…», «+7…» — "
    "недостающая 7 добавляется сама. Есть диалог с этим номером в Umnico — "
    "пишет в него, нет — пишет первым в канал из поля channel: «max» (по "
    "умолчанию) или «telegram». Какой канал — СПРАШИВАЙ У КЛИЕНТА: «вам "
    "удобнее в MAX или в Телеграм?» (правило Ирика 25.09.2026). Правило от "
    "21.09.2026: пообещал прислать — отправляй В ЭТОМ ЖЕ разборе этим "
    "инструментом; фото — ссылкой на папку живых материалов из брифа "
    "(вложения канал не принимает, только текст). Обязательно chat_id и "
    "account чата Авито, из которого пришёл номер, и deal_id — факт "
    "отправки пишется в карточку.",
...
async def max_send(args: dict) -> dict:
    ...
    канал = (args.get("channel") or "max").strip().lower()
    if канал in ("тг", "телеграм", "telegram", "tg"):
        канал = "telegram"
    elif канал in ("макс", "max", ""):
        канал = "max"
    else:
        return _err(f"канал {args.get('channel')!r} не знаю — «max» или «telegram»")
    try:
        у = Umnico()
        try:
            итог = у.отправить(номер, текст, канал=канал)
        finally:
            у.close()
    except UmnicoError as e:
        назв = "MAX" if канал == "max" else "Телеграм"
    ...
    _sent.append({"chat_id": args.get("chat_id"),
                  "text": f"[{метка} +{номер}] {текст}", "канал": канал})
    if args.get("deal_id"):
        try:
            bitrix().timeline_comment_add(
                args["deal_id"],
                f"#{метка} · отправлено на +{номер} ({итог['как']}) · "
                f"{datetime.now(МСК):%d.%m %H:%M}\n\n{текст}",
            )
